...
//modified my xxh for D+ -> pi0 K+ Ks :a0(980)
...
//    fptype k0mass  = 0.497614;
	fptype KsMass = 0.497611;
	fptype KpMass = 0.493677;
	fptype etamass = 0.547862;
	fptype pipMass = 0.13957061;
...
//        fpcomplex retur(A * C, B * C);
	   ...
	   fptype rhoetapi = 2*twoBodyCMmom(s, KsMass, KpMass)/sqrt(s);
	   fptype rhoKKbar = 2*twoBodyCMmom(s, etamass, pipMass)/sqrt(s);
	   fptype img = rhoetapi*g1 + rhoKKbar*g2;

	   fpcomplex retur = fpcomplex(1,0)/fpcomplex(resmass*resmass-s,-img);
        ret += retur;
        if(doSwap) {
            fptype swpmass = m12;
            m12            = m13;
