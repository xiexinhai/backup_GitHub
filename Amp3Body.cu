...
__host__ fptype Amp3Body::normalize() {
    ...
    thrust::constant_iterator<fptype *> arrayAddress_ff(dalitzNormRange);
    thrust::counting_iterator<int> binIndex(0);
    thrust::counting_iterator<int> binIndex_ff(0);
        ...
        for(int j = 0; j < decayInfo.resonances.size(); ++j) {
		  ...
		  // Added by xxh for fit fractions
	       ...
	       integrators_ff[i][j]->setNoEff();
            ...
            integrators_ff[i][j]->setResonance2Index(decayInfo.resonances[j]->getFunctionIndex());
            thrust::constant_iterator<int> effFunc_useless(efficiencyFunction);
            fpcomplex dummy_ff(0, 0);
            thrust::plus<fpcomplex> complexSum_ff;
            (*(integrals_ff[i][j])) = thrust::transform_reduce(
                thrust::make_zip_iterator(thrust::make_tuple(binIndex_ff, arrayAddress_ff, effFunc_useless)),
                thrust::make_zip_iterator(thrust::make_tuple(binIndex_ff + totalBins, arrayAddress_ff, effFunc_useless)),
                *(integrators_ff[i][j]),
                dummy_ff,
                complexSum_ff);
        }
